    ...
    <div class="max-w-6xl mx-auto px-4 py-6 flex gap-4">
        ...
        <section class="flex-1 p-4 md:p-6">
            ...
            <div class="flex items-center justify-between mb-4 flex-wrap gap-2">
                <div>
                    <h1 class="text-xl md:text-2xl font-semibold">Data Dosen</h1>
                    <p class="text-sm text-slate-500">Kelola data dosen pembimbing.</p>
                </div>
                <a href="<?= BASE_URL; ?>/?r=admin/dosenCreateForm"
                ...
                <table class="min-w-full text-sm">
                    <thead class="bg-slate-50">
                        <tr>
                            ...
                            <th class="px-4 py-2 text-left text-xs font-semibold text-slate-500">NIP</th>
                            ...
                            <th class="px-4 py-2 text-left text-xs font-semibold text-slate-500">No. HP</th>
                        ...
                        <?php else: ?>
                            <?php $no = 1; foreach ($items as $row): ?>
                                <tr class="border-t">
                                    ...
                                    <td class="px-4 py-2 font-mono text-xs"><?= htmlspecialchars($row['nip']); ?></td>
                                    ...
                                    <td class="px-4 py-2 text-right">
                                        <div class="inline-flex items-center space-x-1">
                                            <!-- Edit -->
                                            <a href="<?= BASE_URL; ?>/?r=admin/dosenEditForm&id=<?= $row['id']; ?>"
                                            ...
                                            <!-- Delete -->
                                            <form action="<?= BASE_URL; ?>/?r=admin/dosenDelete" method="post" class="inline"
                                              onsubmit="return confirm('Yakin ingin menghapus dosen ini beserta akun loginnya?');">
                                            <?= $this->csrfField() ?>
                                            <input type="hidden" name="id" value="<?= $row['id'] ?>">
                                            <button type="submit" class="p-1.5 bg-red-600 rounded hover:bg-red-700 flex items-center justify-center">
